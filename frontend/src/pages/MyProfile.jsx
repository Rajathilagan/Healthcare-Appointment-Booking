...
const MyProfile = () => {
  const [userData, setUserData] = useState({
    ...
    address: {
      line1: '1 st street marathahalli',
  ...
  return (
    <div className='max-w-lg flex flex-col gap-2 text-sm'>
      ...
      <div>
        {
          isEdit
            ? <button className='border border-primary px-8 py-2 rounded-full mt-3 text-sm hover:bg-primary hover:text-white transition-all duration-500' onClick={() => setIsEdit(false)}>Save Information</button>
            : <button className='border border-primary px-8 py-2 rounded-full mt-3 text-sm hover:bg-primary hover:text-white transition-all duration-500' onClick={() => setIsEdit(true)}>Edit</button>
        }
      </div>
    </div>
  )
